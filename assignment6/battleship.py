...
class Ship:
    def __init__(self, name, start_position, orientation):
        ...
        if orientation == 'v':
            for i in range(SHIP_SIZES.get(name)):
                self.positions[chr(ord(start_position[0])+i), start_position[1]] = self.sunk

        elif orientation == 'h':
            for i in range(SHIP_SIZES.get(name)):
                self.positions[(start_position[0], (start_position[1]+i))] = self.sunk
...
class Game:
    ...
    def in_bounds(self, start_position, ship_size, orientation):
        ...
        """
        if orientation == 'h':
            if (start_position[1] + (ship_size-1)) <= 9:
                return True
            ...
            if (ord(start_position[0]) + (ship_size-1)) <= ord('J'):
                return True
    ...
    def overlaps_ship(self, start_position, ship_size, orientation):
        ...
        if orientation == 'h':
            for item in self.ships:
                for key in item.positions:
                    if key[0] == start_position[0]:  # if same row
                        for i in range(start_position[1], (start_position[1] + ship_size)):
                            if key[1] == i:
                                return True
            return False
        ...
        if orientation == 'v':
            for item in self.ships:
                for key in item.positions:
                    if key[1] == start_position[1]:  # if same column
                        for i in range(ord(start_position[0]), ord(start_position[0]) + ship_size ):
                            if key[0] == chr(i):
                                return True
            return False
    ...
    def place_ship(self, start_position, ship_size):
        ...
        elif self.in_bounds(start_position, ship_size, 'v') and not self.overlaps_ship(start_position, ship_size, 'v'):
            return 'v'
    ...
    def create_and_place_ships(self):
        ...
        for key in SHIP_SIZES:
            ship_size = SHIP_SIZES[key]
            valid_placement = 0
            start_position = 0

            while valid_placement != 'h' and valid_placement != 'v':
                start_position = get_random_position()
                valid_placement = self.place_ship(start_position, ship_size)

            ship = Ship(key, start_position, valid_placement)
            self.ships += [ship]
    ...
    def get_guess(self):
        ...
        """
        row = 0
        while row < 65 or row > 74:
            row = ord(input("Enter a row: "))

        column = -1
        while column < 0 or column > 9:
            column = int(input("Enter a column: "))

        position = (chr(row), column)
    ...
    def check_guess(self, position):
        ...
        """
        for item in self.ships:
            if position in item.positions and item.positions[position] == False:
                item.positions[position] = True

                if False not in item.positions.values():
                    print("You sunk the " + str(item.name) + "!")
                    item.sunk = True
    ...
    def update_game(self, guess_status, position):
        ...
        """
        if guess_status:
            row = self.board.get(position[0])
            row[position[1]] = 'x'
            self.board[position[0]] = row
...
def end_program():
    ...
    while stop_game != "Y" and stop_game != "y" and stop_game != "N" and stop_game != "n":
        stop_game = input("Play again (Y/N)?")

    if stop_game.upper() == "Y":
        return False

    elif stop_game.upper() == "N":
        return True
